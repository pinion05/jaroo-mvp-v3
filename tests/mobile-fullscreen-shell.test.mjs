...
test('mobile Jaroo shell uses the full viewport instead of a framed phone card', () => {
  assert.match(shellSource, /min-h-dvh bg-white text-foreground/)
  assert.match(shellSource, /min-h-dvh w-full flex-col overflow-hidden bg-white/)
...
test('DeepScan loading handoff fills mobile before desktop framing resumes', () => {
  assert.match(deepScanPageSource, /flex min-h-dvh justify-center bg-white sm:bg-\[color:var\(--jaroo-canvas\)\] sm:px-6 sm:py-4/)
