...
test('DeepScan inline conclusion waits until visible team summaries finish', () => {
  ...
  assert.match(source, /function buildFallbackTeamSummary/)
  assert.match(source, /const TEAM_SUMMARY_MAX_LENGTH = 88/)
  assert.match(source, /function compactSummaryLine/)
  assert.match(source, /requestTeamSummary/)
  assert.match(source, /attempt < 2/)
  assert.match(source, /근거 요약/)
  assert.doesNotMatch(source, /요약 생략/)
  assert.doesNotMatch(source, /팀 요약을 불러오지 못했어요/)
