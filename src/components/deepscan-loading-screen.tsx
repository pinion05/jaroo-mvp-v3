...
export function DeepScanLoadingScreen({
  ...
  const progressLabel = canShowInlineResults
    ? '상세 결과 준비 완료'
    : !activeNarrativeCard
      ? '오늘 장 흐름을 먼저 정리하는 중…'
  ...
  return (
    <div className={cn(styles.loadingCard, className)}>
      <header className={styles.topBar}>
        ...
        <div className={cn(styles.headerProgress, canShowInlineResults ? styles.headerProgressDone : undefined)} aria-label={canShowInlineResults ? '딥스캔 완료' : '딥스캔 진행 중'}>
          ...
          <span className={styles.headerProgressTime}>{canShowInlineResults ? '완료' : formatElapsedTime(elapsedSeconds)}</span>
        </div>
      </header>
      ...
      <div className={styles.body}>
        <section className={styles.intro} aria-label='딥스캔 안내'>
          ...
          <p className={styles.introBody}>{canShowInlineResults ? '실제 응답이 도착했어요. 아래 결과 카드가 바로 이어집니다.' : '완료 신호가 오면 기다림 없이 이 화면 아래에 결과가 이어집니다.'}</p>
        </section>
        ...
        <details className={styles.progressDetails}>
          ...
          <div className={styles.stepsWrap} aria-label='분석 단계'>
            {[
              ...
              { label: '세 팀 분석 대기', state: canShowInlineResults ? 'done' : evidenceCollected ? 'active' : 'wait' },
              { label: '상세 리포트 연결', state: canShowInlineResults ? 'done' : 'wait' },
            ].map((step, index) => {
              ...
              return (
                <div key={step.label} className={styles.stepRow}>
                  ...
                  {index === 2 ? <div className={styles.stepCount}>{canShowInlineResults ? '완료' : `${pendingCommitteeMemberCount}명 대기`}</div> : null}
                </div>
              )
            })}
          </div>
          ...
          <section className={styles.committeeWrap} aria-label='세 팀 분석 진행 상태'>
            <div className={styles.committeeTitle}>{canShowInlineResults ? '세 팀 분석 완료' : '세 팀 분석 대기 중'}</div>
            <div className={styles.membersGrid}>
              {committeeMembers.map((member) => (
                <div key={member.key} className={styles.member}>
                  <div className={cn(styles.memberIcon, memberStateClass(canShowInlineResults ? 'done' : member.state))}>
        ...
        {canShowInlineResults && inlineResults ? <div className={styles.inlineResultsSlot}>{inlineResults}</div> : null}
