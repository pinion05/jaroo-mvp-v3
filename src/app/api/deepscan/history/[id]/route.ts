...
import { getScanHistoryById } from '@/lib/deepscan-history-store'
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await resolveApiUserId('deepscan-history-detail')
  ...
  const row = await getScanHistoryById(auth.userId, String(id ?? ''))
  if (!row) {
