...
| 시나리오 확률·목표가 | 안내 카드 — **`원천 부재`: "ETF에는 애널리스트 목표가·컨센서스가 없어요"** | **구성종목 가중 목표가로 교체** ("구성종목 컨섴서스 기준 예상 레벨" — 목표가 아님을 명시. 확률%는 미표시) | AI 위원(별도 스펙) |
| 구성종목 테이블 | 안내 카드 — `소스 준비 중` | **실데이터** (ETFComponent + 구성 코드 일괄 quotes) | |
