...
| D7 | 미지원 필드 표시 | 채울 수 없는 필드는 **이유를 화면에 명시** (단순 "준비 중" 아님). 사유 3종: `원천 부재`(ETF엔 존재하지 않는 데이터 — 시나리오 확률), `소스 준비 중`(기술적 과제 — 섹터 비중·추적오차·모멘텀 섹터 문구), `출시 후 예정`(배당·비교 ETF) |
...
| 모멘텀 라벨 | **재정의** — "섹터 순풍" 문구 폐기, 일둥 기반 자체 모멘텀(최근 1주 등락) | | 섹터 모멘텀(매핑 확보 시) |
| 기본정보: 운용사·기준지수·총보수·순자산·NAV | **실데이터** (etf-profile, D6 승격) | | |
| 기본정보: 설정일·유형·배당수익률 | 응답에 있으면 실데이터, **없으면 해당 행 스킵**(플레이스홀더 아님) | | |
| 수익 지표(1M/3M/6M/1Y) | 안내 카드 — `소스 준비 중`(일둥 파이프라인 연결 전) | **실데이터** (일봉 산출) | |
| 시나리오 확률·목표가 | 안내 카드 — **`원천 부재`: "ETF에는 애널리스트 목표가·컨센서스가 없어요"** | **52주 범위 위치로 교체**(현재가의 저점~고점 범위 내 위치 % 표기. 카드 노트에 애널리스트 목표가 부재 사유 상시 명시) | 구성종목 가중 목표가 **3단계 이관**(실행 중 판명: 구성별 컨센서스 크롤 13페이지×N건은 페이지 로드에 부적합 — 경량 소스 확보 과제, #265 기록) → AI 위원(별도 스펙) |
| 구성종목 테이블 | 안내 카드 — `소스 준비 중` | **실데이터** (ETFComponent. 구성등락률 열은 quotes/current에 등락률 부재로 미제공 — 비중 바 중심, 요약행에 사유 명시) | 구성등락률 소스 확보 시 |
| 섹터 비중 | 안내 카드 — `소스 준비 중`(구성종목×섹터 매핑 과제) | 매핑 확보 시 실데이터, 미확보 시 안내 카드 유지(#265 기록) | |
| 리스크: 변동성·MDD·샤프·괴리율 | 안내 카드 — `소스 준비 중` | **실데이터** (일봉 산출 / 괴리율은 price API) | |
| 리스크: 추적오차·베타 | 안내 카드 — `소스 준비 중`(NAV 시계열·시장지수 일둥 미확보) | 안내 카드 유지 | 소스 확보 시 |
| 배당·비교 ETF | 안내 카드 — `출시 후 예정` | 안내 카드 유지 | 실데이터 |
...
- `src/components/etf-data-notice-card.tsx` — 미지원 블록 안내 카드 (D7): 사유 라벨(`원천 부재`/`소스 준비 중`/`출시 후 예정`) + 블록별 사유 문구를 명시 렌더. 예: "ETF에는 애널리스트 목표가·컨섴서스가 없어요"
